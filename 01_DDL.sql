/* 실행은 ctrl + enter
데이터베이스 생성 명령
데이터베이스 내부에 테이블들이 적재되기 때문에 먼저 데이터 베이스 생성
DEFAULT CHARACTER SET UEF8; 을 붙여주시면 한글 설정이 됩니다.
...
bitcamp06이 생성된게 확인되면 우클릭 -> set as default schemas 선택 시
볼드 처리되고, 지금부터 적는 쿼리문은 해당 DB에 들어간다는 의미임. */

/* 해당 DB에 접근할 수 있는 사용자 계정 생성 생성 
...
모든 테이블의 컬럼중 하나는 반드시 PK속성이 부여되어 있어야 함.
...
해당 테이블에 적재된 데이터를 조회할 수 있습니다.
SELECT (컬럼명1, 컬럼명2, ...) FROM 테이블명;
이용해서 특정 컬럼에 적재된 데이터만 조회할 수도 있습니다. */
SELECT * FROM users;

/* 문제
3번 유저를 임의로 추가해주세요.
조회 구문은 이메일 회원번호, 아이디 순으로 세 컬럼만 조회하는 구문 */
INSERT INTO users VALUES (3, 'def2345', '김디비', 'def@db.com');
...
-- INSERT INTO 구문으로 7, 8번 유저를 추가해주시고
-- 주소는 둘 다 '강남구'로 넣으려고 시도해보세요.
INSERT INTO users VALUES (7, 'cccc', '강남불백', '강남구');
INSERT INTO users VALUES (8, 'dddd', '브롱스', '강남구');
SELECT * FROM users;

-- 주소에 걸린 unique 제약을 없애고 8번 추가
-- 위에서 u_address컬럼에 걸린 제약조건이 별칭인 u_address_unique를 삭제
