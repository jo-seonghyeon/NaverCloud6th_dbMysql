...
SELECT user_address, user_name FROM user_tbl WHERE user_address='서울' OR user_address='경기';

-- IN문법은 IN다음에 오는 리스트 목록에 포함된 요소만 출력해줍니다.
SELECT user_address, user_name FROM user_tbl WHERE user_address IN ('서울', '경기');

-- IN 문법을 응용해서 구매내역이 있는 유저만 출력해보겠습니다.
SELECT user_num FROM buy_tbl; -- 왼쪽 구문이 구매자 번호만 나타내는 리스트
-- 서브쿼리를 활용한 조회구문
...
SELECT * FROM user_tbl WHERE user_height = null; -- 안되는 구문
